...
scores <- function(obs, sim, alts, dates, est){
  ...
  # set the same NAs in sim as obs
  for(i in 1:ncol(obs)){
    wna <- which(is.na(obs[,i]))
    if(length(wna) > 0) sim[wna, i] <- NA
  }
  ...
  ###################
  message('Computing statistics by quantiles')
  errors <- data.frame(matrix(NA, nrow = 10, ncol = 7), 
                       stringsAsFactors=F)
  names(errors) <- c('%OBS', '%PRED', 'MAE', 'ME', 'RM', 'RSD', 'Range')
  rownames(errors) <- paste('d', 1:10, sep = '')
  ...
  # lo <- log10(ol)
  # lsi <- log10(sl)
  # hist(lo, main = '', xlab = 'logPcp (mm)', freq = F, xlim = c(min(lo, lsi, na.rm=T), max(lo, lsi, na.rm=T)))
  ...
  #altitude stats
  ####################   
  message('Computing statistics by altitudes')
  #breaks
  brks <- cut(est$ALT, alts)
  errors <- data.frame(matrix(NA, nrow = length(levels(brks)),
                              ncol = 7),
                       stringsAsFactors = F)
  ...
  for(i in 1:length(levels(brks))){
    ...
    wna <- which(is.na(o))
    if(length(wna) > 0) {o <- o[-c(wna)]; s <- s[-c(wna)]}
    # wna <- which(is.na(s))
  ...
  pdf('./val/medias_medians_95_estaciones_y_dias.pdf', width=10, height=6, pointsize = 12)
    ...
    legend('bottomright',
           bty = 'n',
           legend = c(paste('Pearson', 
                            round(cor(om1, pm1, use = "pairwise.complete.obs"), 
                                  3), ', ME', round(mean(pm1-om1, na.rm=T),2))))
    abline(0,1) 
    
  # medians wet
  ome <- pme <- o95 <- p95 <- rep(NA, ncol(obs))
  for(i in 1:ncol(obs)){
  ...
  smoothScatter(ome, pme,
                ylim = c(min(ome, pme, na.rm = T),
                         max(ome, pme, na.rm = T)),
                xlim = c(min(ome, pme, na.rm = T),
                         max(ome, pme, na.rm = T)),
                xlab = 'Observed (mm)',
                ylab = 'Predicted (mm)',
                colramp = Lab.palette,
  ...
  legend('bottomright',
         ...
         legend = c(paste('Pearson', 
                          round(cor(ome, pme, 
                                ...
                                3), ', ME', round(mean(pme-ome, na.rm=T),2))))
  abline(0, 1) 
  
  #95th wet
  smoothScatter(o95, p95,
                ylim = c(min(o95, p95, na.rm = T), 
                         max(o95, p95, na.rm = T)),
                xlim = c(min(o95, p95, na.rm = T),
                         max(o95, p95, na.rm = T)),
                xlab = 'Observed (mm)',
                ylab = 'Predicted (mm)',
                colramp = Lab.palette,
  ...
  abline(-sd(p95, na.rm = T), 1, lty = 2, col = colors()[220])
  legend('bottomright',
         bty = 'n', legend = c(paste('Pearson',
                                     round(cor(o95, p95, 
                                               use = "pairwise.complete.obs"),
                                           3), ', ME', round(mean(p95-o95, na.rm=T),2))))
  abline(0, 1) 
  ...
  #medians wet
  ome <- pme <- o95 <- p95 <- rep(NA,nrow(obs))
  for(i in 1:nrow(obs)){
    w <- which(obs[i, ] > 0 & sim[i, ] > 0)
    if(length(w) > 10){
  ...
  legend('bottomright',
         bty = 'n',
         legend = c(paste('Pearson',
                          round(cor(ome, pme, use = "pairwise.complete.obs"),
                                3), ', ME', round(mean(pme-ome, na.rm=T),2))))
  abline(0, 1) 
  ...
  legend('bottomright',
         bty = 'n',
         legend = c(paste('Pearson',
                          round(cor(o95, p95, use = "pairwise.complete.obs"),
                                3), ', ME', round(mean(p95-o95, na.rm=T),2))))
  abline(0, 1) 
  ...
  ##monthly validation
  message('Computing statistics by months')
  ...
  # wna <- apply(omsum, 2, function(x) which(is.na(x)))
  # for(h in 1:nrow(pmsum)){
  #   if(length(wna[[h]]) > 0) pmsum[wna[[h]], h] <- NA
  # }
  ...
  for(i in 1:12){
    ...
    k <- as.numeric(as.matrix(k))
    ko <- as.numeric(as.matrix(ko))
    
    errorsMon[i,1] <- round(mae(k, ko), 2)
    errorsMon[i,2] <- round(me(k, ko),2)
    errorsMon[i,3] <- round(mean(k, na.rm = T) / mean(ko, na.rm = T),2)
    errorsMon[i,4] <- round(sd(k, na.rm = T) / sd(ko, na.rm = T), 2)
    errorsMon[i,5] <- round(cor(k, ko, use = "pairwise.complete.obs"), 2)
    errorsMon[i,6] <- paste(round(min(as.numeric(as.matrix(omsum[w, ])), na.rm = T),1),
                            'to',
                            round(max(as.numeric(as.matrix(omsum[w, ])), na.rm = T),1))
  }
  write.table(errorsMon,'./val/monthly_stats.txt',sep='\t',quote=F,row.names=F)
  
  #tabla de contingencia
  ####################################
  message('Computing contingency table')
  
  onum <- as.numeric(as.matrix(obs))
  pnum <- as.numeric(as.matrix(sim))
  contTable <- matrix(NA, 2, 2)

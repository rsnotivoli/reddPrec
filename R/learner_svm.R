#' Function of the support vector machine model (svm)

#' @description The function of the support vector machine model (svm) is used to predict wet/dry (classification) days and the amount of rain on wet days (regression). This function employs the hybrid model of Serrano-Notivoli et al. (2017).
...
#' To use this function, the e1071 package should have been installed previously
...
learner_svm <- function(ref, can, covars) {
  ...
  if (length(unique(rr$val)) != 1) {
    ...
    fmtb <- e1071::svm(f, data = rr[c("val", covars)], kernel = "radial",
                       ...
                       probability = TRUE)
    pb <- predict(fmtb, as.data.frame(can[, covars]), probability = TRUE)

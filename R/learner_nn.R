#' Function of the neural network model (nn)

#' @description The function of the neural network model (nn) is used to predict wet/dry (classification) days and the amount of rain on wet days (regression). This function employs the hybrid model of Serrano-Notivoli et al. (2017).
...
#' To use this function, the neuralnet package should have been installed previously
...
learner_nn <- function(ref, can, covars) {
  ...
  if (length(unique(rr$val)) != 1) {
    ...
    fmtb <- neuralnet::neuralnet(
      ...
      hidden = c(4, 2),
      stepmax = 1e6,
      threshold = 0.75 # threshold worked well with 0.5 in CH, changed to 0.75 by MP-SA
    )
    pb <- predict(fmtb, as.data.frame(can[, covars]))
    pb <- pb[, 1]
  ...
  fmt <- neuralnet::neuralnet(
    ...
    data = rr[, c("val", covars)],
    hidden = c(4, 2),
    stepmax = 1e6, # stepmax problems with 0.01 (negative values with 0.1)
    # act.fct = custom,
    threshold = 0.75 # threshold worked well with 0.5 in CH, changed to 0.75 by MP-SA
  ) 
  ...
  e <- sqrt(
    sum(
      (rr$val - predict(fmt,
                        ...
                        type = "response")) ^ 2) / (length(rr$val) - length(covars))
    )
  e <- (e * RANGE) + MINc

qc_after_hmg <- function(target_data, mm_apply_qc = 0, apply_qc) {
  ...
  time_step <- time(oro_hmg[oro_hmg[, "hmg"] > mm_apply_qc])
  ...
  dates_time_n <-
    data.frame(
      hmg_val = as.numeric(target_data$hmg_time_series[time_step][time(n_cubic_dff)]),
      oro_val = as.numeric(target_data$raw_time_series[time_step][time(n_cubic_dff)]),
  ...
  new_target_data$hmg_time_series[time(new_hmg_val)] <- new_hmg_val
